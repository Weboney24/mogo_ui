...
const NewArivals = () => {
  ...
  const handleOpen = async (value) => {
    try {
      ...
      successMessage("Products Added to New Arivals");
      fetchData();
    } catch (err) {
      console.log(err);
  ...
  const handleRemoveOffer = async (value) => {
    try {
      setLoading(true);
      let formdata = {
        new_arival_status: false,
        product_id: value._id,
      };
      await updateProductRequestStatus(formdata);
      successMessage("Products Removed to New Arivals Products");
      fetchData();
    } catch (err) {
      errorMessage("Failed to Removed Product from New Arivals Products");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex h-full justify-between gap-x-2 w-full pt-2">
                  ...
                  >
                    ...
                    <div
                      className={`offer_style_button ${
                        item.new_arival_status
                          ? "bg-primary !text-white cursor-not-allowed"
                          : ""
                      }`}
                      onClick={() => {
                        item.new_arival_status ? " " : handleOpen(item);
                      }}
                    >
                      {item.new_arival_status
                        ? "Already Added"
          ...
          <div className="!w-[40%] h-full bg-white p-2 rounded-lg shadow-lg">
            <h1 className="px-2 py-1 center_div justify-start gap-x-2">
              <IconHelper.newArivals /> New Arival Products
            </h1>
            <div className="w-full flex items-center justify-start flex-wrap gap-2">
              {allProducts
                ?.filter((res) => {
                  return res.new_arival_status === true;
                })
                .map((item, index) => {
                  return (
                    <div
